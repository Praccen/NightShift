...
export module CollisionSolver {
	...
	function updateMomentum(
		...
		movComp: MovementComponent,
		inf: IntersectionTester.IntersectionInformation,
		entity: Entity,
		axis: vec3,
		change: vec3
	) {
		if (collisionComp.isDynamic) {
			let collisionPointOffset = vec3.subtract(
				vec3.create(),
				inf.point,
				ECSUtils.CalculatePosition(entity)
			);
		...
		for (let inf of intersectionInformation) {
			...
			if (dotProd < 0.0) {
				...
				let collisionCoefficient = Math.max(
					e1CollisionComp.collisionCoefficient,
					e2CollisionComp.collisionCoefficient
				); // TODO: This can be calculated differently, will be based on material abilities in the future
				let frictionCoefficient = Math.max(
					e1CollisionComp.frictionCoefficient,
					e2CollisionComp.frictionCoefficient
				); // TODO: This can be calculated differently, will be based on material abilities in the future
				...
				if (
					...
					let frictionMagnitude = Math.min(
						relativeTangentVel * frictionCoefficient,
